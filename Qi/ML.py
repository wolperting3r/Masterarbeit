...
def build_model():
    ...
    model = tf.keras.Sequential([
        layers.Dense(128, activation='relu', kernel_initializer='he_normal', input_shape=(9,)),
...
def train_model(model, train_data, train_labels, regenerate=False):
    ...
    dataset = tf.data.Dataset.from_tensor_slices((train_data, train_labels)).batch(4)
    if regenerate:
        # Train Model
        ...
        model.fit(dataset, verbose=0, callbacks=[TqdmCallback(verbose=1)], epochs=1000)
