...
def generate_data(N_values, st_sz, equal_kappa, neg, silent=False):
    ...
    for n in range(N_values):
        ...
        if equal_kappa:
            ...
            r = 1/(-curvature*e) + u()**4*((e**2)/(-curvature) - 1/(-curvature*e))
            # print(f'\nkappa:\t{curvature}\n e:\t{e}\n r:\t{r}')
        else:
            # Get random radius
        ...
        pt_x = pm()*np.sqrt((((-e**2*r**2)/curvature)**(2/3)-r**2)/(e**4-e**2))
        ...
        round_point = np.floor(x*L/Delta)*Delta/L
	    ...
	    # Initialize plot
            ...
            # Plot circle
            plot_ellipse(ax1, r, e, x, rot, curvature=curvature)
        # '''
