...
def generate_data(N_values, stencils, ek, neg, silent, geometry, smearing, usenormal, interpolate):
    ...
    for n in range(N_values):
        ...
        if smearing:
            ...
            if interpolate:
                # Linear interpolation between data before and after smearing
                a = interpolate*u()
                # Cut edges of vof_array
                vof_array = vof_array[1:st_sz_tmp[0]-1, 1:st_sz_tmp[1]-1]
                vof_array_smear = vof_array_smear[1:st_sz_tmp[0]-1, 1:st_sz_tmp[1]-1]
                # Interpolate
                vof_array = vof_array + a*(vof_array_smear-vof_array)
            else:
                # Cut edges of vof_array
