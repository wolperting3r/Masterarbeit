...
def tecplot2data(f, st_sz, filtering, filter):
    print(f'Generating data from {f} with stencil {st_sz[0]}x{st_sz[1]}')
    with open('CVOFLS_Rawdata_' +f+'.dat', 'r') as myfile:
        data = myfile.read()
    ...
    output = np.empty((0, np.prod(st_sz)+1))
    ...
    for b in bl.keys():
        ...
        new_data = np.concatenate((kappa, np.reshape(stencils, (stencils.shape[0], stencils.shape[1]*stencils.shape[2]))), axis = 1)
    ...
    file_name = os.path.join(parent_path, 'data_CVOFLS_'+f+'_'+str(st_sz[0])+'x'+str(st_sz[1])+(f'_{filter}{filtering}' if filtering else '')+'.feather')
    print(f'File:\n{file_name}')
...
kwargs = {'f': files, 'st_sz': st_sz, 'filtering': filtering, 'filter': filter}
