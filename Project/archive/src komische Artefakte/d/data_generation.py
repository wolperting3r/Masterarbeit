...
def generate_data(N_values, stencils, ek, neg, silent, geometry, smearing, usenormal):
    ...
    if not visualize:
        ...
        if geometry == 'ellipse':
            ...
            geom_str = '_cir'
        # Create file name
        file_name = os.path.join(parent_path, 'data', 'datasets', 'data_'+str(st_sz[0])+'x'+str(st_sz[1])+('_eqk' if equal_kappa else '_eqr')+('_neg' if neg else '_pos')+geom_str+('_smr' if smearing else '_nsm')+'_shift1'+('' if usenormal else 'b')+'_n.feather')
        print(f'File:\n{file_name}')
        # Export file
