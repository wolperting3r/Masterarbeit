...
class Rotate(BaseEstimator, TransformerMixin):
    ...
    def transform(self, dataset):
        ...
        '''
        # Get indices where vof values should be rotated/flipped along y=-x (rot 0 is extended by epsilon in both directions)
        epsilon = 0.02
        # rot01 = np.argwhere((mid_angle >= 0) & (mid_angle <= 0+epsilon))
        rot270f = np.argwhere((mid_angle > 0+epsilon) & (mid_angle <= 1/8))
        rot270 = np.argwhere((mid_angle > 1/8) & (mid_angle <= 1/4))
        rot180f = np.argwhere((mid_angle > 1/4) & (mid_angle <= 3/8))
        rot180 = np.argwhere((mid_angle > 3/8) & (mid_angle <= 1/2))
        rot90f = np.argwhere((mid_angle > 1/2) & (mid_angle <= 5/8))
        rot90 = np.argwhere((mid_angle > 5/8) & (mid_angle <= 3/4))
        rot0f = np.argwhere((mid_angle > 3/4) & (mid_angle <= 7/8-epsilon))
        # rot02 = np.argwhere((mid_angle > 7/8-epsilon) & (mid_angle <= 1))
        # '''
        ...
        print_data_nrt = data[rot0][ind].reshape((st_sz[0], st_sz[1])).copy()
        # '''
        ...
        print_data_rot = data[rot0][ind].reshape((st_sz[0], st_sz[1])).copy()
        print(f'Data before:\n{print_data_nrt}')
        print(f'Data after:\n{print_data_rot}')
        # '''
