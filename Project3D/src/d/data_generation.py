...
def axismatrix(n_grid, axis):
    ...
    if axis == 0:
        return return_matrix.transpose((0, 2, 1))
    elif axis == 1:
        return return_matrix.transpose((2, 0, 1))
    elif axis == 2:
...
def cross(mid_pt, max_pt, rev_y=False):
    ...
    if rev_y:
        ...
        mid_pt = [-mid_pt[0], mid_pt[1], mid_pt[2]]

    # Get limits in x-direction
    points_x = np.array([np.array([mid_pt[0]]), mid_pt[1]+max_pt[1], np.array([mid_pt[2]])])
    # Get limits in y-direction
    points_y = np.array([mid_pt[0]+max_pt[0], np.array([mid_pt[1]]), np.array([mid_pt[2]])])
    # Get limits in z-direction
    points_z = np.array([np.array([mid_pt[0]]), np.array([mid_pt[1]]), mid_pt[2]+max_pt[2]])
    ...
    cr_z = np.array(list(itertools.product(*points_z)))
    # Combine them into a list
    cross_points = np.unique(np.concatenate((cr_y, cr_x, cr_z), axis=0), axis=0)
...
def generate_data(N_values, stencils, ek, neg, silent, geometry, smearing, usenormal):
    ...
    st_mid = [int((st_sz[0]-1)/2), int((st_sz[1]-1)/2), int((st_sz[2]-1)/2)]
    cr_mid = [int((cr_sz[0]-1)/2), int((cr_sz[1]-1)/2), int((cr_sz[2]-1)/2)]
    ...
    st_crp = np.array([np.arange(-cr_mid[0], (cr_mid[0]+1)),
                       ...
                       np.arange(-cr_mid[2], (cr_mid[2]+1))])*Delta
    ...
    st_stp = np.array([np.arange(-st_mid[0], (st_mid[0]+1)),
                       np.arange(-st_mid[1], (st_mid[1]+1)),
                       np.arange(-st_mid[2], (st_mid[2]+1))])*Delta
    ...
    for n in range(N_values):
        ...
        if geometry == 'sinus':
            ...
            phi = 2*np.pi*u()
            theta = np.pi*u()  # np.acos(2*u()-1)
            # Get cartesian coordinates on sphere surface
            x_rel = np.array([r*np.sin(theta)*np.sin(phi),   # y
                              r*np.cos(theta)*np.sin(phi),   # x
                              r*np.cos(phi)])  # z
        ...
        vof_array = np.empty((cr_sz[0], cr_sz[1], cr_sz[2]))
        vof_array[:] = np.nan

        # '''
        if visualize:
            # Create pandas dataframe to fetch shape of geometry in local coordinates
            vof_df = pd.DataFrame(index=range(np.prod(st_sz)), columns=['1'])
        ...
        for idx, ill in enumerate(cross_point_indices):
            ...
            [y_l, x_l, z_l] = np.array([local_grid[0] + lo[0] - x_c[0],
            ...
            if visualize:
                # Save the r_area array (containing the shape of the geometry) for plotting
                    vof_df.iloc[ill[0]*st_sz[0]**2+ill[1]*st_sz[1]+ill[2], 0] = r_area

        # Calculate gradient with central finite difference:
        grad_y = vof_array[cr_mid[0]+1, cr_mid[1], cr_mid[2]]-vof_array[cr_mid[0]-1, cr_mid[1], cr_mid[2]]
        grad_x = vof_array[cr_mid[0], cr_mid[1]+1, cr_mid[2]]-vof_array[cr_mid[0], cr_mid[1]-1, cr_mid[2]]
        grad_z = vof_array[cr_mid[0], cr_mid[1], cr_mid[2]+1]-vof_array[cr_mid[0], cr_mid[1], cr_mid[2]-1]
        # Calculate normal vector
        normal = -1/np.sqrt(grad_y*grad_y+grad_x*grad_x+grad_z*grad_z)*np.array([grad_y, grad_x, grad_z])

        # Shift round point by random amount to cover points around the interface as well
        if usenormal:
            shift_vector = normal
        else:
            shift_vector = np.array([-grad_y, -grad_x, -grad_z])
        ...
        if True:
            ...
            if geometry == 'sinus':
                ...
                # Plot circle
                plot_circle(ax1, r, x_c+shift_point*shift_vector*Delta/L, x)
            # plt.show()
        #'''

        # Initialize new variables
        # Initialize vof_array and vof_df with stencil size
        vof_array = np.empty((st_sz[0], st_sz[1], st_sz[2]))
        vof_array[:] = np.nan
        if visualize:
            # Create pandas dataframe to fetch shape of geometry in local coordinates
            #vof_df = pd.DataFrame(index=range(st_sz[0]), columns=range(st_sz[1]))
            vof_df = pd.DataFrame(index=range(np.prod(st_sz)), columns=['1'])
            print(f'vof_df.shape:\t{vof_df.shape}')
        ...
        local_origin_points = np.array([round_point[0]+st_stp_tmp[0],
                                        ...
                                        round_point[2]+st_stp_tmp[2]])
        # Get list of all origins in stencil
        local_origins = np.array(list(itertools.product(
            *[local_origin_points[0],  # [::-1] to get sorting right
              local_origin_points[1],
              local_origin_points[2]]
        )))
        # Get list of all stencil indices combinations
        local_indices = np.array(list(itertools.product(range(st_sz_tmp[0]), range(st_sz_tmp[1]), range(st_sz_tmp[2]))))  # g
        ...
        for idx, ill in enumerate(local_indices):
            ...
            if np.isnan(vof_array[ill[0], ill[1], ill[2]]):
                # Get origin coordinates too
        ...
        if smearing:
            ...
            print(f'vof_array[2-1:2+2, 2-1:2+2, 2-1:2+2].shape:\n{vof_array[2-1:2+2, 2-1:2+2, 2-1:2+2].shape}')
            tt = np.sum(np.multiply(kernel, vof_array[2-1:2+2, 2-1:2+2, 2-1:2+2]))/np.sum(kernel)
            print(f'tt:\n{tt}')
            print(f'tt.shape:\n{tt.shape}')
            ...
            for column in range(1, st_sz_tmp[0]-1):
                for row in range(1, st_sz_tmp[1]-1):
                    for zz in range(1, st_sz_tmp[2]-1):
                        # Calculate smeared vof field: sum(weights * vof_array_slice)/sum(weights)
                        vof_array_smear[column, row, zz] = np.sum(np.multiply(
                            kernel,
                            vof_array[column-1:column+2, row-1:row+2, zz-1:zz+2]
                        ))/np.sum(kernel)
            # Cut edges of vof_array
            vof_array = vof_array_smear[1:st_sz_tmp[0]-1, 1:st_sz_tmp[1]-1, 1:st_sz_tmp[2]-1]
            #if visualize:
                # Cut vof_df too
                #vof_df = vof_df.iloc[1:-1, 1:-1, 1:-1]
            # Shrink st_sz
            st_sz_tmp = np.add(st_sz_tmp, [-2, -2, -2])
            st_mid_tmp = np.add(st_mid_tmp, [-1, -1, -1])
        print(f'vof_array:\n{vof_array}')
        ...
        if (vof_array[st_mid_tmp[0], st_mid_tmp[1], st_mid_tmp[2]] > 0) & (vof_array[st_mid_tmp[0], st_mid_tmp[1], st_mid_tmp[2]] < 1):
            '''
                e = 100
            if True:
            # '''
