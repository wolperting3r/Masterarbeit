...
def plot_circle(ax1, r, x_c, x):
    ...
    phi_plt = 1*np.pi*values
    theta_plt, phi_plt = np.mgrid[0:2*np.pi:20j, 0:np.pi:10j]
    # Get x and y values of circle edge
    # !! x_c = [-x_c_y, x_c_x]
    [x_plt, y_plt, z_plt] = [r*np.cos(theta_plt)*np.sin(phi_plt)+x_c[1],
          r*np.sin(theta_plt)*np.sin(phi_plt)-x_c[0],
          r*np.cos(phi_plt)+x_c[2]]
    # Plot circle
    #ax1.fill(x_plt, y_plt, color='w', zorder=-1)
    ax1.plot_wireframe(x_plt, y_plt, z_plt, facecolors='b', alpha=0.05)
    ax1.plot_wireframe(x_plt, y_plt, z_plt, color='b')
    # Plot point [-y, x]
    ax1.scatter(x[1], -x[0], x[2], color='r')
    ax1.set_facecolor('k')
    # Print radius 
    #ax1.text(0.05, 0.05, 0.05, f'r = {np.round(r,3)}', transform=ax1.transAxes, color='r')
    ...
    for column in range(st_sz[0]):  # y
        ...
        for row in range(st_sz[1]):  # x
            if len(column_values) == 0:
                column_values = vof_df.iloc[row*st_sz[0]**2+column*st_sz[1]+int((st_sz[0]-1)/2), 0][:, :, int((st_sz[0]-1)/2)]
                #column_values = np.flip(vof_df.iloc[row*st_sz[0]**2+column*st_sz[1]+int((st_sz[0]-1)/2), 0][:, :, int((st_sz[0]-1)/2)], axis=1)
            else:
                column_values = np.concatenate((column_values, vof_df.iloc[row*st_sz[0]**2+column*st_sz[1]+int((st_sz[0]-1)/2), 0][:, :, int((st_sz[0]-1)/2)]), axis=1)
                #column_values = np.concatenate((column_values, np.flip(vof_df.iloc[row*st_sz[0]**2+column*st_sz[1]+int((st_sz[0]-1)/2), 0][:, :, int((st_sz[0]-1)/2)], axis=1)), axis=1)
        # Glue rows together
        if len(image) == 0:
            image = column_values
        else:
    ...
    for row in range(vs[1]):
        for column in range(vs[0]):
            ...
            ax2.text(txt_pt[0], txt_pt[1],
                 np.round(vof_array[column, row, int((st_sz[0]-1)/2)], 3),
